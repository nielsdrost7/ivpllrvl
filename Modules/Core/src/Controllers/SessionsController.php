    ...
    public function passwordreset($token = null)
    {
        ...
        if (request()->input('btn_reset', true)) {
            ...
            if ($recovery_result = $this->db->where('user_email', $email)) {
                ...
                //use salt to prevent predictability of the reset token (CVE-2021-29023)$token = md5(time() . $email . $this->crypt->salt());
                ...
                if (MailerHelper::mailerConfigured()) {if ( ! phpmail_send($email_from, $email, trans('password_reset'), $email_message)) {
                        $email_failed = true;
                    }
                } else {// Set email configuration
