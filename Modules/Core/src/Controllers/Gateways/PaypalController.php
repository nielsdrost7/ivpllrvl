...
class PaypalController extends BaseController
{
    ...
    {
        // Check if the invoice exists and is billable$invoice = $this->mdl_invoices->where('ip_invoices.invoice_url_key', $invoice_url_key)->get()->row();
    ...
    public function paypalCapturePayment(string $order_id)
    {
        ...
        if ($paypal_response['status']) {
            ...
            //record the payment$this->mdl_payments->save(null, ['invoice_id' => $invoice_id, 'payment_date' => date('Y-m-d'), 'payment_amount' => $amount, 'payment_method_id' => get_setting('gateway_paypal_payment_method'), 'payment_note' => '']);
    ...
    {//load the REST API consumer library, 'client_secret' => $this->crypt->decode(get_setting('gateway_paypal_clientSecret')), 'demo' => get_setting('gateway_paypal_testMode') == 1], 'lib_paypal');
    }
}
