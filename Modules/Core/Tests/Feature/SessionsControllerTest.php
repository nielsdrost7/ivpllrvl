...
class SessionsControllerTest extends TestCase
{
    ...
    public function it_authenticates_user_with_valid_credentials(): void
    {
        ...
        $response = $this->post(route('sessions.login.post'), [
            'btn_login' => true,
    ...
    public function it_rejects_authentication_for_nonexistent_user(): void
    {
        ...
        $response->assertRedirect(route('sessions.login'));
        $response->assertSessionHas('alert_error', trans('core::messages.loginalert_user_not_found'));
        $this->assertGuest();
    }
    ...
    public function it_rejects_authentication_for_inactive_user(): void
    {
        ...
        $response->assertRedirect(route('sessions.login'));
        $response->assertSessionHas('alert_error', trans('core::messages.loginalert_user_inactive'));
        $this->assertGuest();
    }
    ...
    public function it_throttles_login_attempts_after_multiple_failures(): void
    {
        ...
        for ($i = 0; $i < 10; $i++) {
            $this->post(route('sessions.login.post'), [
                'btn_login' => true,
                'email' => 'test@example.com',
                'password' => 'wrongpassword',
            ]);
    ...
    public function it_sends_password_reset_email(): void
    {
        ...
        $response = $this->post(route('sessions.passwordreset.post'), [
            'btn_reset' => true,
            'email' => 'test@example.com',
        ]);
    ...
    public function it_throttles_password_reset_attempts(): void
    {
        ...
        for ($i = 0; $i < 10; $i++) {
            $this->post(route('sessions.passwordreset.post'), [
                'btn_reset' => true,
                'email' => 'test@example.com',
            ]);
        }
    ...
    {
        $response = $this->get(route('sessions.login', ['token' => 'invalid_token']));
    ...
    {
        $response = $this->get(route('sessions.login', ['token' => 'token<script>alert(1)</script>']));
